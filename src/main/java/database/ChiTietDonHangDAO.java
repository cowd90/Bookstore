...
public class ChiTietDonHangDAO implements IDAO<ChiTietDonHang> {
	...
	public ArrayList<ChiTietDonHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				String maChiTietDonHang = rs.getString("machitietdh");
				String donhang = rs.getString("madonhang");
				String sanpham = rs.getString("masp");
	...
	public ChiTietDonHang selectById(ChiTietDonHang t) {
		...
		try {
			...
			String sql = "SELECT * FROM chitietdh WHERE machitietdh=?";
			PreparedStatement st = con.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				String sanpham = rs.getString("masp");// o
				int soluong = rs.getInt("soluong");
				double giagoc = rs.getDouble("giagoc");
				double giamgia = rs.getDouble("giamgia");
				double giaban = rs.getDouble("giaban");
	...
	public int insert(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "INSERT INTO chitietdh"
					+ " VALUES (?,?,?,?,?,?,?,?,?)";
	...
	public int delete(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "DELETE from chitietdh " + " WHERE machitietdh=?";
	...
	public int update(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE chitietdh SET madonhang=?, masp=?, soluong=?, giagoc=?, giamgia=?, giaban=?, vat=?, tongtien=?"
					+ " WHERE machitietdh=?";
